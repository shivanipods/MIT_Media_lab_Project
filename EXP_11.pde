...
void draw()
{
  ...
  if(keyPressed == true)
  {
    ...
    //if(key == 'h') player.rewind();
    if(key == 'f') player.skip(500);
    if(key == 'k') player.skip(-500);
    if(key == 'q') player.close();
    if(key == 'l') player.setBalance(-1);
    if(key == 'r') player.setBalance(1);
    if(key == 'n') player.setBalance(0);
  }
